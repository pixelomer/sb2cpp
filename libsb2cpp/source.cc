...
void Source::visit_subroutine(AST::Subroutine *subroutine) {
    // Subroutines are handled by the constructor.
    throw std::runtime_error("visit_subroutine() should never be called");
}
...
void Source::visit_stdlib_assign(AST::StdlibAssign *assign) {
    ...
    auto rvalue_var = dynamic_cast<AST::VariableValue *>(assign->value.get());
...
void Source::visit_goto_label(AST::GotoLabel *label) {
    // Goto labels are handled by visit_statement_group().
    // Doing double dispatch causes shared_ptr to be inaccessible.
    throw std::runtime_error("visit_goto_label() should never be called");
}
